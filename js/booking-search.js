...
document.addEventListener('input', (event) => {
  ...
  const query = normalize(input.value); const rows = [...section.querySelectorAll('[data-booking-id]')]; let visible = 0;
  ...
  section.querySelectorAll('[data-booking-day]').forEach((group) => { group.hidden = ![...group.querySelectorAll('[data-booking-id]')].some((row) => !row.hidden); });
