...
const state = {
  ...
  analysisRange: 'month', analysisCategoryId: null, analysisMonth: null, theme: 'light', financialMonthMode: 'calendar', financialMonthStart: 28
};
let viewRoot, bottomNavRoot;
await bootstrap();
...
async function bootstrap() {
  ...
  syncFinancialMonthStorage(); applyTheme(state.theme, false); await reloadData(); mountShell(); bindGlobalEvents(); render(); requestAnimationFrame(() => showOnboarding());
}
function mountShell() { app.innerHTML = `<div id="view-root"></div><div id="bottom-nav-root"></div>`; viewRoot = app.querySelector('#view-root'); bottomNavRoot = app.querySelector('#bottom-nav-root'); bottomNavRoot.innerHTML = renderBottomNav(state.view); }
async function reloadData() { const projectionWindow = projectionWindowForMonth(state.month); const [allBookings,budgets,categoryMap] = await Promise.all([listBookingsWithProjections(projectionWindow),listBudgetsForMonth(state.month),getCategoryMap()]); const startDay = state.financialMonthMode === 'custom' ? state.financialMonthStart : 1; state.allBookings=allBookings; state.bookings=filterBookingsForPeriod(allBookings,getFinancialMonthPeriod(state.month,startDay)); state.budgets=budgets; state.categoryMap=categoryMap; }
function render() { const views={overview:()=>renderOverview(state),analysis:()=>renderAnalysis(state),budgets:()=>renderBudgets(state),settings:()=>renderSettings(state)}; viewRoot.innerHTML=(views[state.view]??views.overview)(); updateBottomNav(); }
...
function resetAnalysisSelection() { state.analysisCategoryId=null; state.analysisMonth=null; }
