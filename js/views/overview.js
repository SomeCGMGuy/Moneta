...
export function renderOverview({ month, bookings, allBookings, categoryMap }) {
  ...
  const timelineBookings = allBookings.filter((booking) => booking.date <= `${month}-31`);
  return `<main class="page" data-active-month="${escapeHtml(month)}">
    ...
    <section class="section" data-booking-section><div class="section-heading"><div><h2>Buchungen</h2><p class="section-subtitle">Chronologisch durch deine Finanzhistorie</p></div><span class="chip" data-booking-visible-count>${Math.min(timelineBookings.length, countFirstMonths(timelineBookings, 3))}</span></div>${timelineBookings.length ? `<label class="booking-search" aria-label="Buchungen durchsuchen"><span class="booking-search-icon" aria-hidden="true">⌕</span><input type="search" data-booking-search placeholder="Buchungen durchsuchen" autocomplete="off" spellcheck="false" /></label>` : ''}${renderBookingList(timelineBookings, categoryMap, { timeline: true, pageSize: 3 })}<div class="card empty-state booking-search-empty" data-booking-search-empty hidden><strong>Keine Treffer</strong>Für diese Suche gibt es in den geladenen Buchungen keinen Treffer.</div></section>
  </main>`;
}

function countFirstMonths(bookings, limit) {
  const months = [...new Set(bookings.map((booking) => String(booking.date).slice(0, 7)))].sort().reverse().slice(0, limit);
  return bookings.filter((booking) => months.includes(String(booking.date).slice(0, 7))).length;
}
