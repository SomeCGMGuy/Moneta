...
export function renderBookingList(bookings, categoryMap, { currentBookingIds = null } = {}) {
  ...
  return `<div class="booking-groups">${groups.map(({ date, bookings: rows }) => `
    <section class="booking-day" data-booking-day>
      ...
      <div class="card booking-list">${rows.map((booking) => renderRow(booking, categoryMap, currentBookingIds)).join('')}</div>
    </section>`).join('')}</div>`;
}

function renderRow(booking, categoryMap, currentBookingIds) {
  ...
  const current = currentBookingIds ? currentBookingIds.has(booking.id) : true;
  return `<${tag} class="booking-row${booking.isProjected ? ' booking-row-projected' : ''}" ${interaction} data-booking-row data-booking-date="${escapeAttr(booking.date)}" data-booking-current="${current ? 'true' : 'false'}" data-booking-search="${escapeAttr(searchText)}"${current ? '' : ' hidden'}>
