...
export async function showBookingForm({ booking = null }) {
  ...
  const updateRecurrenceDisplay = () => {
    const option = RECURRENCE_OPTIONS.find((row) => row.value === recurrenceInput.value) ?? RECURRENCE_OPTIONS[0];
    layer.querySelector('[data-recurrence-value]').textContent = option.label;
    recurrenceEndField.hidden = recurrenceInput.value === 'none';
  };
  ...
  layer.querySelector('[data-recurrence-picker]').addEventListener('click', async () => {
    const value = await showChoiceSheet({ title: 'Wiederholung', options: RECURRENCE_OPTIONS, selected: recurrenceInput.value });
    if (!value) return;
  ...
  form.addEventListener('submit', (event) => {
    ...
    navigation.close({ id: booking?.id, type: data.get('type'), amount: data.get('amount'), categoryId: data.get('categoryId'), title: data.get('title'), note: data.get('note'), date: data.get('date'), recurrenceFrequency: data.get('recurrenceFrequency'), recurrenceEndDate: data.get('recurrenceEndDate') });
  });
...
function buildMarkup(booking, type, categories, defaultDate) {
  ...
  return `<section class="push-page" role="dialog" aria-modal="true" aria-labelledby="booking-page-title">
    ...
    <form class="push-page-form"><div class="push-page-content booking-editor">
      ...
      <input type="hidden" name="recurrenceFrequency" value="${escapeAttr(recurrenceFrequency)}" />
      ...
      <div class="field"><label>Kategorie</label><button class="native-select-row" type="button" data-category-picker><span class="native-select-value" data-category-value>${escapeHtml(initialCategory ? `${initialCategory.icon || '•'} ${initialCategory.name}` : 'Kategorie wählen')}</span><span class="native-select-chevron" aria-hidden="true">›</span></button></div>
      ...
      <div class="field recurrence-field"><label>Wiederholung</label><button class="native-select-row" type="button" data-recurrence-picker><span class="native-select-value" data-recurrence-value></span><span class="native-select-chevron" aria-hidden="true">›</span></button><p class="form-hint">Zukünftige Termine werden als Prognose in Salden und Statistiken berücksichtigt.</p></div>
